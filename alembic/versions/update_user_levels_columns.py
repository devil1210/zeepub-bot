...
def upgrade():
    ...
    op.execute(
        "UPDATE user_levels SET daily_downloads = -1, early_access = TRUE, custom_themes = TRUE, price = 0.0 WHERE id = 1"
    )  # Admin
    op.execute(
        "UPDATE user_levels SET daily_downloads = -1, early_access = TRUE, custom_themes = TRUE, price = 0.0 WHERE id = 2"
    )  # Staff
    op.execute(
        "UPDATE user_levels SET daily_downloads = 100, early_access = TRUE, custom_themes = TRUE, price = 5.0 WHERE id = 3"
    )  # Premium
    op.execute(
        "UPDATE user_levels SET daily_downloads = 50, early_access = TRUE, custom_themes = FALSE, price = 2.0 WHERE id = 4"
    )  # VIP
    op.execute(
        "UPDATE user_levels SET daily_downloads = 25, early_access = FALSE, custom_themes = FALSE, price = 1.0 WHERE id = 5"
    )  # Patrocinador
    op.execute(
        "UPDATE user_levels SET daily_downloads = 1, early_access = FALSE, custom_themes = FALSE, price = 0.0 WHERE id = 6"
    )  # Lector
