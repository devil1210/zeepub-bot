...
def check_migrations():
    ...
    try:
        with engine.connect() as conn:
            ...
            # 1. books columns (Local Library)
            if table_exists("books"):
                add_column_if_missing("books", "author_jap", "VARCHAR(512)")
                add_column_if_missing("books", "illustrator", "VARCHAR(512)")
                add_column_if_missing("books", "illustrator_jap", "VARCHAR(512)")
                add_column_if_missing("books", "romaji_title", "VARCHAR(512)")
                add_column_if_missing("books", "spanish_title", "VARCHAR(512)")
                add_column_if_missing("books", "english_title", "VARCHAR(512)")
                add_column_if_missing("books", "jap_title", "VARCHAR(512)")
                add_column_if_missing("books", "series_spanish", "VARCHAR(255)")
                add_column_if_missing("books", "series_english", "VARCHAR(255)")
                add_column_if_missing("books", "is_uncensored", "INTEGER DEFAULT 0")
                add_column_if_missing("books", "color_mode", "VARCHAR(50)")
                add_column_if_missing("books", "series_id", "INTEGER")
                add_column_if_missing("books", "source_id_col", "UUID")
                add_column_if_missing("books", "cover_original", "VARCHAR(1024)")
                add_column_if_missing("books", "cover_high", "VARCHAR(1024)")
                add_column_if_missing("books", "cover_medium", "VARCHAR(1024)")
                add_column_if_missing("books", "cover_low", "VARCHAR(1024)")
                add_column_if_missing("books", "summary", "VARCHAR(1024)")
                add_column_if_missing("books", "demographics", "JSONB")
                add_column_if_missing("books", "short_link", "VARCHAR(20) UNIQUE")
