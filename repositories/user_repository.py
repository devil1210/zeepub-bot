...
class UserRepository(BaseRepository[Dict[str, Any]]):
    ...
    # Métodos específicos
    async def upsert(self, telegram_id: int, role: str, expires_at: Optional[datetime] = None, 
                     custom_status: Optional[str] = None, created_by: Optional[int] = None):
        async with self.db.connection() as conn:
            # Check existence
            cursor = await conn.execute("SELECT telegram_id FROM users WHERE telegram_id = ?", (telegram_id,))
    ...
    async def update_status(self, telegram_id: int, custom_status: str):
        async with self.db.connection() as conn:
             await conn.execute("UPDATE users SET custom_status = ? WHERE telegram_id = ?", (custom_status, telegram_id))
             await conn.commit()
