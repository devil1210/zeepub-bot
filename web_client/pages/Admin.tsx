...
export const Admin: React.FC<AdminProps> = ({ onNavigate }) => {
  ...
  // State for user permissions editing navigation
  const [savingUserPerms, setSavingUserPerms] = useState(false);
  const [canUndoPerms, setCanUndoPerms] = useState(false);
  const [canApplyPerms, setCanApplyPerms] = useState(false);

  // Refs for callback functions from UserPermissions component
        ...
        <div className="hidden md:flex items-center gap-2 bg-white/5 p-1.5 rounded-2xl border border-white/5 backdrop-blur-md">
          {viewOptions.map((option) => (
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${currentView === option.id
                ...
                <div className="glass-panel p-6 rounded-3xl border border-white/5 relative overflow-hidden group">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="h-2 bg-white/5 rounded-full overflow-hidden border border-white/5">
                      <div
                        className="h-full bg-gradient-to-r from-primary to-primary rounded-full shadow-[0_0_10px_rgba(var(--primary-rgb),0.3)] transition-all duration-1000"
      ...
      <div className="md:hidden fixed bottom-6 left-8 right-8 z-50 animate-in slide-in-from-bottom-4 duration-300 max-w-7xl mx-auto">
        ...
        >
          {(selectedUserId || configuringTier) ? (
            ...
            /* Normal Admin Mode: Salir | Monitor | Infraestructura | Membresías */
            <>
              <button
