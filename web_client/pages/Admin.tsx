                ...
                <div className="space-y-4 flex-1">
                  {[
                    ...
                    <div key={i} className="p-4 rounded-xl bg-black/20 border border-white/5 hover:border-primary/40 transition-all cursor-pointer group shadow-inner">
                      ...
                      <button className={`w-full py-2.5 rounded-lg text-[10px] font-black tracking-widest transition-all uppercase ${item.sec
        ...
        {currentView === 'tiers' && (
          <div className="max-w-[1200px] mx-auto w-full flex flex-col gap-10 animate-in fade-in duration-300 px-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {levels.map((level) => (
                <div key={level.id} className="glass-panel p-6 rounded-2xl border border-white/5 relative overflow-hidden group hover:border-primary/30 transition-all flex flex-col">
                  <div className="absolute top-0 right-0 p-4 opacity-5">
                    {level.priority >= 9 ? <Medal className="w-16 h-16 text-yellow-500" /> : level.priority >= 4 ? <Gem className="w-16 h-16 text-primary" /> : <User className="w-16 h-16 text-gray-400" />}
                  </div>
                  <div className="relative z-10 flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-3 h-3 rounded-full" style={{ backgroundColor: level.color }}></div>
                      <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Prioridad: {level.priority}</span>
                    </div>
                    <h3 className="text-2xl font-black text-white mb-4">{level.name}</h3>

                    <div className="space-y-4 mb-6">
                      <div className="flex items-center justify-between group/item">
                        <div className="flex items-center gap-2">
                          <TrendingUp className="w-3.5 h-3.5 text-primary" />
                          <span className="text-xs text-gray-300">Descargas Diarias</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <input
                            type="number"
                            ...
                            onChange={(e) => {
                              ...
                              setLevels(prev => prev.map(l => l.id === level.id ? { ...l, dailyDownloads: val } : l));
                            }}
                            className="w-16 bg-black/40 border border-white/10 rounded px-2 py-1 text-xs text-center text-white focus:outline-none focus:border-primary"
                          />
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-gray-300">Acceso Anticipado</span>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                            checked={level.earlyAccess}
                            onChange={(e) => {
                              const val = e.target.checked;
                              setLevels(prev => prev.map(l => l.id === level.id ? { ...l, earlyAccess: val } : l));
                            }}
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-gray-300">Temas Personalizados</span>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                            checked={level.customThemes}
                            onChange={(e) => {
                              const val = e.target.checked;
                              setLevels(prev => prev.map(l => l.id === level.id ? { ...l, customThemes: val } : l));
                            }}
                            className="sr-only peer"
                          />
                          <div className="w-9 h-5 bg-gray-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-primary"></div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-gray-300">Acceso Mini App</span>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                            ...
                            onChange={(e) => {
                              const val = e.target.checked;
                              setLevels(prev => prev.map(l => l.id === level.id ? { ...l, hasAccess: val } : l));
                            }}
                            className="sr-only peer"
                          />
                    ...
                    <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-gray-400 px-1">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setLevels(prev => prev.map(l => l.id === level.id ? { ...l, price: val } : l));
                        }}
                        className="w-16 bg-transparent border-b border-white/10 text-right text-white focus:outline-none focus:border-primary"
                      />
                    </div>
                    <button
                      onClick={async () => {
                      ...
                      className="w-full py-2.5 rounded-xl bg-white/5 hover:bg-primary hover:text-white text-[10px] font-black uppercase tracking-widest text-gray-400 transition-all border border-white/10 hover:border-primary shadow-sm flex items-center justify-center gap-2 group/btn"
                    >
                      <Save className="w-3 h-3 group-hover/btn:scale-110 transition-transform" />
                      Guardar Cambios
                    </button>
                  </div>
                </div>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {[
                      ...
                      <tr key={i} className="hover:bg-white/5 transition-colors group cursor-pointer" onClick={() => setSelectedUserId(row.id)}>
                        ...
                        <td className="p-4">
                          <span className={`px-2 py-1 rounded text-[10px] font-black uppercase tracking-wider ${row.tier === 'VIP' ? 'bg-purple-500/20 text-purple-300 border border-purple-500/20' :
