            ...
            <div className={`grid gap-6 grid-cols-2 ${actions.length > 4 ? 'md:grid-cols-3' : 'sm:grid-cols-4'}`}>
                {actions.map((item) => (
                    ...
                    >
                        ...
                        <div className={`relative z-10 p-5 rounded-[1.6rem] ${item.bg} ${item.color} border border-white/5 shadow-inner group-hover:scale-110 group-hover:-translate-y-2 transition-all duration-1000`}>
                            <item.icon className="w-8 h-8" strokeWidth={2.5} />
                        </div>
                        <div className="relative z-10">
                            <span className="block text-white font-black text-[13px] uppercase tracking-[0.15em] mb-1.5 drop-shadow-sm group-hover:text-primary transition-colors duration-500">{item.label}</span>
                            <span className="block text-gray-500 text-[10px] font-bold uppercase tracking-[0.2em] opacity-50 group-hover:opacity-100 transition-opacity duration-700">{item.desc}</span>
                        </div>
                    </button>
                ))}
            </div>
