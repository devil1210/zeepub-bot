...
export const BookDetail: React.FC<BookDetailProps> = ({
  ...
  const handleDirectDownload = async () => {
    ...
    try {
      ...
      // Crear iframe temporal o redirección nativa para activar la descarga del navegador
  ...
  const getRomajiTitle = (): string => {
    // 1. Usar romaji_title del volumen (LocalBook)
    const volumeRomaji = cleanRomajiText(String(curVolume.romaji_title || curVolume.romajiTitle || curVolume.romaji || ''));
    if (volumeRomaji) return volumeRomaji;

    // 2. Usar name / series_name / romajiTitle de la serie (fallback)
    const seriesRomaji = cleanRomajiText(String(curSeries?.romajiTitle || curSeries?.romaji || curSeries?.name || curSeries?.series_name || curSeries?.seriesName || ''));
  ...
  const displayData = {
    ...
    demography: (Array.isArray(curVolume.demographics) ? curVolume.demographics : (Array.isArray(curVolume.demography) ? curVolume.demography : (Array.isArray(curSeries?.demographics) ? curSeries.demographics : []))).map((d: any) => typeof d === 'object' && d !== null ? d.name : String(d)),
    genres: (Array.isArray(curVolume.tags) ? curVolume.tags : (Array.isArray(curVolume.genres) ? curVolume.genres : (Array.isArray(curSeries?.genres) ? curSeries.genres : []))).map((g: any) => typeof g === 'object' && g !== null ? g.name : String(g)),
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
            ...
            <div className="lg:col-span-8 xl:col-span-9 flex flex-col gap-6">
              ...
              <BookHeader
                ...
                spanishTitle={displayData.spanishTitle}
