...
export const BookCard = memo<BookCardProps>(({ book, onDownload, compact = false }) => {
  ...
  return (
    <div className="group relative glass-panel rounded-premium-lg overflow-hidden border border-white/5 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10 transition-all duration-300 flex flex-col h-full bg-slate-900/50">
      <div className={`relative overflow-hidden ${compact ? 'aspect-[3/4]' : 'aspect-[2/3]'} shrink-0 bg-black/80`}>
        <div className="absolute top-3 left-3 z-20 flex flex-col gap-1.5">
          <span className="bg-black/60 shadow-lg backdrop-blur-md text-white text-[9px] font-black px-2 py-1 rounded-premium-sm uppercase tracking-widest border border-white/10">
            {book.book_type || book.format}
          </span>
          {book.is_uncensored && (
            <span className="bg-red-600/90 text-white text-[8px] font-black px-2 py-0.5 rounded-premium-sm uppercase tracking-widest shadow-lg border border-white/10 w-min">
              S/C
            </span>
          )}
        </div>
      ...
      <div className="p-3 flex flex-col flex-1 gap-2 relative z-10">
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 text-yellow-500 bg-yellow-500/10 px-1.5 py-0.5 rounded-premium-sm border border-yellow-500/10">
            <Star className="w-3 h-3 fill-current" />
            <span className="text-[10px] font-black tracking-wider text-yellow-500">{book.rating > 0 ? book.rating.toFixed(1) : '-'}</span>
          </div>
          <span className="text-[9px] font-mono text-gray-500 uppercase px-1.5 py-0.5 bg-white/5 rounded-premium-sm border border-white/5">{book.size}</span>
        </div>
        ...
        <div className="mt-auto pt-2 flex items-center justify-between">
          <button
            ...
            className="w-full py-1.5 bg-white/5 hover:bg-primary text-gray-400 hover:text-white rounded-premium-md flex items-center justify-center gap-2 transition-all group/btn"
          >
