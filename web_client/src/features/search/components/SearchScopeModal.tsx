...
export const SearchScopeModal: React.FC<SearchScopeModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center pointer-events-auto">
      ...
      {/* Modal Content */}
      <div className="relative w-full max-w-md glass-panel rounded-t-premium-lg sm:rounded-premium-lg border-white/10 shadow-premium transform transition-all animate-in slide-in-from-bottom-20 fade-in duration-500 flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="w-full flex justify-center pt-4 pb-2 relative z-10">
          <div className="w-16 h-1.5 bg-white/10 rounded-premium-full shadow-inner"></div>
        </div>
        ...
        <div className="p-8 pt-4 relative z-10">
            ...
            {scopes.map((scope) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-4.5 rounded-premium-lg transition-all duration-500 relative group overflow-hidden ${isSelected
                    ? 'bg-primary/20 border-primary/30 text-white shadow-premium shadow-blue-500/10'
                ...
                >
                  ...
                  <div className="flex items-center gap-5 relative z-10">
                    <div className={`p-3 rounded-premium-lg ${isSelected ? 'bg-primary text-white shadow-premium shadow-blue-500/30' : 'bg-white/5 text-gray-600 group-hover:text-gray-300'} transition-all duration-500`}>
                      <scope.icon className="w-5 h-5" strokeWidth={2.5} />
                    </div>
                    <span className={`text-[13px] uppercase tracking-[0.2em] transition-all duration-500 ${isSelected ? 'font-black scale-105' : 'font-bold opacity-60 group-hover:opacity-100'}`}>{scope.label}</span>
                  </div>
          ...
          <button
            onClick={onClose}
            className="w-full mt-8 py-5 rounded-premium-lg bg-white/5 border border-white/10 text-gray-400 font-black text-[10px] uppercase tracking-[0.3em] hover:bg-white/10 hover:text-white transition-all duration-300 shadow-premium"
          >
            Cerrar Panel
          </button>
        </div>
