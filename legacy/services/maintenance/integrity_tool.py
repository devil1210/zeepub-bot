    ...
    async def run(self, progress_callback=None, **kwargs) -> dict[str, Any]:
        ...
        try:
            async with pg_manager.get_session() as session:
                ...
                stmt = select(LocalBook.series_id).where(LocalBook.series_id.is_not(None)).distinct()
                ...
                if total == 0:
                    logger.info("Integrity check: No series hashes found in books.")
                    return {
                        "success": True,
                        "stats": stats,
                ...
                logger.info(f"Integrity check: Processing {total} series hashes...")

                for i, s_hash in enumerate(hashes):
                    # 2. Re-verificar la serie (buscando por hash semántico)
                    ...
                    if series:
                        ...
                        # En v4 la vinculación es directa por hash
                    else:
                        stats["skipped"] += 1
                        logger.debug(f"Integrity: No SeriesMetadata found for hash {s_hash}")

                    if progress_callback:
                        await progress_callback(i + 1, total, f"Verificando serie {s_hash[:8]}...")
            ...
            logger.info(f"✅ Integrity tool finished. Processed {stats['series_hashes_processed']} series hashes.")
            return {
                "success": True,
                "stats": stats,
                "finished_at": datetime.utcnow().isoformat(),
