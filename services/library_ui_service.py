...
async def mostrar_series(update: Update, context: ContextTypes.DEFAULT_TYPE, origin_type: str, filter_val: str = None, page: int = 1):
    ...
    nav_row = []
    if page > 1:
        nav_row.append(InlineKeyboardButton("⬅️ Ant.", callback_data=f"nav_p|{origin_type}|{filter_val or ''}|{page-1}"))
    if page * page_size < data["total"]:
        nav_row.append(InlineKeyboardButton("Sig. ➡️", callback_data=f"nav_p|{origin_type}|{filter_val or ''}|{page+1}"))
