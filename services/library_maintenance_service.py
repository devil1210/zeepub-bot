...
class LibraryMaintenanceService:
    ...
    def get_library_stats() -> Dict[str, any]:
        ...
        try:
            ...
            # Tipos de libro
            book_types = session.query(
                LocalBook.book_type,
                func.count(LocalBook.id)
            ).group_by(LocalBook.book_type).all()
            
            # Tamaño total de archivos
            total_file_size = session.query(func.sum(LocalBook.file_size)).scalar() or 0
            
            # Portadas
            books_with_covers = session.query(LocalBook).filter(LocalBook.cover_path.isnot(None)).count()
            
            # Tamaño de la base de datos
            db_size = os.path.getsize(DB_PATH)
            
            # Tamaño del directorio de portadas
            covers_size = sum(f.stat().st_size for f in Path(COVERS_DIR).rglob('*') if f.is_file()) if Path(COVERS_DIR).exists() else 0
