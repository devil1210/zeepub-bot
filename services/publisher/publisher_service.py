...
class TelegramPublisherProvider(PublisherProvider):
    ...
    async def announce_book(self, 
        ...
        from utils.helpers import formatear_mensaje_portada, escapar_html, generar_slug_from_meta
        ...
        sinopsis = book_data.get("description") or book_data.get("summary") or book_data.get("sinopsis")
        ...
        if not sinopsis:
             ...
             if series_id:
                 from services.metadata_service import obtener_sinopsis_opds, obtener_sinopsis_opds_volumen
                 try:
                     if volume_id:
        ...
        if options.get("with_buttons", True):
            ...
            if not keyboard:
                keyboard = [
                    [
                        InlineKeyboardButton("📥 Descargar", callback_data=f"descargar_confirm"),
            ...
            try:
                ...
                if "thread not found" in str(e).lower():
                    await self.bot.send_message(chat_id=target_id, text=info_text, parse_mode="HTML")
                    await self.bot.send_message(chat_id=target_id, text="¿Deseas descargar este libro?", reply_markup=InlineKeyboardMarkup(keyboard))
...
class FacebookPublisherProvider(PublisherProvider):
    ...
    async def announce_book(self, 
        ...
        from utils.helpers import validate_facebook_credentials, formatear_metadata_fb, formatear_titulo_fb
        import httpx
        import logging
