        ...
        for i, s1 in enumerate(all_series):
            for _j, s2 in enumerate(all_series[i + 1 :], start=i + 1):
                ...
                if (
                    ...
                    s1_dict = {
                        "series_name": s1.series_name,
                        "author": s1.author,
                        "book_count": s1.book_count,
                    }
                    s2_dict = {
                        "series_name": s2.series_name,
                        "author": s2.author,
...
async def main():
    ...
    while True:
        ...
        if series_to_analyze:
            console.print(
                f"[yellow]Analizando lote de {len(series_to_analyze)} series para enriquecimiento...[/yellow]"
            )
