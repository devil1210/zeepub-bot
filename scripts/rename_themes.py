    ...
    try:
        ...
        async with engine.begin() as conn:
            ...
            for old_name, new_name in THEME_RENAMES.items():
                ...
                result = await conn.execute(text("SELECT id FROM app_themes WHERE name = :old_name"), {"old_name": old_name})
                ...
                if theme_to_rename:
                    ...
                    result = await conn.execute(text("SELECT id FROM app_themes WHERE name = :new_name"), {"new_name": new_name})
