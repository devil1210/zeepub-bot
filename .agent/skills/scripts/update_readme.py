...
def update_readme():
    ...
    content = re.sub(r"(Collection of )\d+(\+ Universal)", f"\\g<1>{total_skills}\\g<2>", content)
    ...
    if canonical_collections_header not in content:
        ...
        if registry_header_match:
            ...
            content = (
                content[: registry_header_match.start()]
                + insert_block
                + content[registry_header_match.start() :]
            )
    ...
    content = dedupe_pattern.sub(
        f"{canonical_collections_header}\n\n{canonical_collections_body}\n\n", content
    )
    ...
    for skill in skills:
        ...
        risk_icon = "⚪"
        if risk == "official":
            risk_icon = "🟣"  # Mapping official to purple
        if risk == "none":
            risk_icon = "🟢"
        if risk == "safe":
            risk_icon = "🔵"
