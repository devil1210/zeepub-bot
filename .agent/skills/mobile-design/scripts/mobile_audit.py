...
class MobileAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        primary_buttons = re.findall(
            r'(?:testID|id):\s*["\'](?:.*(?:primary|cta|submit|confirm)[^"\']*)["\']', content, re.IGNORECASE
        )
        ...
        if is_react_native:
            ...
            has_system_font = bool(
                re.search(r"fontFamily:\s*[\"']?(?:System|San Francisco|Roboto|-apple-system)", content)
            )
        ...
        potential_low_contrast = bool(
            re.search(r"#[EeEeEeEe].*#ffffff|#999999.*#ffffff|#333333.*#000000|#666666.*#000000", content)
        )
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [
                d
                for d in dirs
                if d not in {"node_modules", ".git", "dist", "build", ".next", "ios", "android", "build", ".idea"}
            ]
            for file in files:
