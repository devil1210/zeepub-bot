...
async def handle_admin_reset_library(data: dict[str, Any], user_data: dict[str, Any]):
    ...
    try:
        ...
        with engine.begin() as conn:
            for table in ["user_ratings", "user_downloads", "metadata_proposals", "ai_learning_feedback", "local_books", "series_metadata", "library_sources", "duplicate_books", "upload_books"]:
...
async def handle_admin_find_duplicates(data: dict[str, Any], user_data: dict[str, Any]):
    ...
    session = get_session()
    try:
        duplicate_hashes = session.query(LocalBook.book_hash, func.count().label("count")).filter(LocalBook.book_hash.isnot(None)).group_by(LocalBook.book_hash).having(func.count() > 1).all()
        duplicate_groups = []
        total_wasted_space = 0
        for hash_row in duplicate_hashes:
            content_hash = hash_row[0]
            books = session.query(LocalBook).filter(LocalBook.book_hash == content_hash).order_by(LocalBook.indexed_at.asc()).all()
            if len(books) <= 1:
                continue
            ...
            duplicate_groups.append({
                "book_hash": content_hash, "title": books[0].title, "author": books[0].author, "count": len(books),
                "total_size": sum(file_sizes), "wasted_space": wasted,
                "books": [{"id": b.id, "filepath": b.filepath, "filename": b.filename, "file_size": b.file_size or 0, "indexed_at": b.indexed_at.isoformat() if b.indexed_at else None} for b in books]
            })
        duplicate_groups.sort(key=lambda x: x["wasted_space"], reverse=True)
        return {"success": True, "duplicate_groups": duplicate_groups, "summary": {"total_duplicates": len(duplicate_hashes), "wasted_space_mb": round(total_wasted_space / (1024*1024), 2)}}
    except Exception as e:
        return {"success": False, "message": str(e)}
...
async def handle_admin_delete_duplicate(data: dict[str, Any], user_data: dict[str, Any]):
    ...
    try:
        ...
        count = 0
        for book in books:
            if book.filepath and os.path.exists(book.filepath): os.remove(book.filepath)
...
async def handle_admin_delete_duplicate_item(data: dict[str, Any], user_data: dict[str, Any]):
    ...
    try:
        ...
        if target == "original":
            ...
            if book:
                session.add(ArchivedBook(series_hash=book.series_hash, book_hash=book.book_hash, title=book.title, filename=book.filename, last_filepath=book.filepath, original_book_id=book.id, reason="manual_duplicate_resolution"))
                session.delete(book)
        session.delete(dup)
        session.commit()
...
async def handle_admin_get_duplicates(data: dict[str, Any], user_data: dict[str, Any]):
    ...
    try:
        ...
        return {"success": True, "duplicates": [{"id": d.id, "title": d.title, "author": d.author, "hash": d.book_hash, "original": d.original_filepath, "duplicate": d.duplicate_filepath, "detectedAt": d.detected_at.isoformat() if d.detected_at else None} for d in dups]}
    finally: session.close()
...
async def handle_admin_merge_series(data: dict[str, Any], user_data: dict[str, Any]):
    check_staff(user_data)
    try:
        success = await LibraryService.merge_series(data.get("target_hash"), data.get("source_hash"), data.get("new_name"))
...
async def handle_admin_bulk_upload_confirm(data: dict[str, Any], user_data: dict[str, Any]):
    ...
    if not selected_ids and not discarded_ids: raise HTTPException(status_code=400, detail="No IDs provided")
    ...
    for upload_id in selected_ids:
        ...
        try:
            ...
            epub_uploader._log_history(user_id=info["user_id"], filename=info["original_filename"], book_hash=meta.get("book_hash"), status=status, final_path=meta.get("suggested_path") if success else None)
            if success: epub_uploader.cleanup_upload(upload_id, f_path)
            results.append({"upload_id": upload_id, "success": success})
        except Exception as e: results.append({"upload_id": upload_id, "success": False, "error": str(e)})
    return {"success": True, "results": results}

async def handle_get_upload_history(data: dict[str, Any], user_data: dict[str, Any]):
    check_staff(user_data)
    async with pg_manager.get_session() as session:
        stmt = select(UploadHistory).order_by(desc(UploadHistory.created_at)).limit(data.get("limit", 100)).offset(data.get("offset", 0))
        res = (await session.execute(stmt)).scalars().all()
        return {"history": [{"id": i.id, "user_id": i.user_id, "filename": i.filename, "status": i.status, "created_at": i.created_at.isoformat() if i.created_at else None} for i in res]}
...
async def handle_admin_enrich_metadata(data: dict[str, Any], user_data: dict[str, Any]):
    ...
    def run_enrichment_in_thread(scanner_obj):
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try: scanner_obj.enrich_all_metadata()
        except Exception as e:
