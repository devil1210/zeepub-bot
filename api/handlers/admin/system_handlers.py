...
async def handle_admin_update_system(data: dict[str, Any], user_data: dict[str, Any]):
    ...
    try:
        ...
        return {"success": True, "message": "Actualización solicitada. El bot contactará con Watchtower.", "restarting": True}
    except Exception as e:
...
async def handle_admin_send_logs_telegram(data: dict[str, Any], user_data: dict[str, Any]):
    ...
    try:
        ...
        def fmt(t): return datetime.fromtimestamp(t).strftime("%Y%m%d_%H%M")
        file_obj.name = f"logs_{fmt(first_t)}_{fmt(last_t)}.txt"
        await bot_instance.app.bot.send_document(chat_id=user_data.get("user_id"), document=file_obj, caption=f"📄 Logs del Sistema\nPeriodo: {fmt(first_t)} a {fmt(last_t)}", parse_mode="HTML")
