...
async def handle_ai_stats(data: dict[str, Any], user_data: dict[str, Any]):
    ...
    try:
        with get_session() as session:
            ...
            processed_series = (
                session.query(func.count(SeriesMetadata.id))
                .filter(SeriesMetadata.series_spanish.isnot(None))
                .scalar()
            )

            # 2. Total pending proposals
            pending = (
                session.query(func.count(MetadataProposal.id)).filter_by(status="pending").scalar()
            )
