...
async def tunnel_opds(
    ...
    if not url.startswith("http"):
        # Assume relative to OPDS root if not absolute
        # But for safety, we generally expect absolute URLs generated by validity logic
        # If relative, we default to START root for safety unless it's obviously something else
        # Actually context varies. Let's assume the client passes what it got from previous links.
        # If it's a relative path starting with /, append to BASE_URL (assuming BASE_URL is OPDS host)
        # But config.OPDS_SERVER_URL is the real source.
