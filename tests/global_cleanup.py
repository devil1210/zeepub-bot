...
try:
    ...
    series_metadata = session.execute(text("SELECT id, series_name, series_hash, book_count FROM series_metadata")).fetchall()
    updated_count = 0
    
    for s_id, s_name, s_hash, b_count in series_metadata:
        actual_count = session.execute(text("SELECT count(*) FROM local_books WHERE series_hash = :h"), {"h": s_hash}).scalar()
        if b_count != actual_count:
            logger.info(f"Updating {s_name}: {b_count} -> {actual_count}")
            session.execute(text("UPDATE series_metadata SET book_count = :c WHERE id = :id"), {"c": actual_count, "id": s_id})
            updated_count += 1
    ...
    empty_series = session.execute(text("SELECT id, series_name, series_hash, author, description, tags, cover_url, book_type, publisher FROM series_metadata WHERE book_count = 0")).fetchall()
